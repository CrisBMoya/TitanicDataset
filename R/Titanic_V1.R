...
#Train model
GBMModel=gbm(formula=Survived ~ ., data=Trainset, n.trees=100000)
...
which(TreeResult==max(TreeResult))
TreeResult[which(TreeResult==max(TreeResult))[1]]
Chosen=gsub(pattern='\\..*', replacement='', x=names(which(TreeResult==max(TreeResult))[1]))

#Other metrics
confusionMatrix(factor(ifelse(test=Predicted[Chosen]>=0.5, yes=1, no=0)),
...
#Save
write_delim(x=data.frame('PassengerId'=TestDF$PassengerId, 'Survived'=SubPred), path='R/Sub_V1.csv', delim=',')
...
if(RE){
  ...
  system('kaggle competitions submit -c titanic -f R/Sub_V1.csv -m "V1 Submission from API"')
}
