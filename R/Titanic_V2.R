...
#GridSearch for distribution
DistSearch=c('gaussian','laplace','tdist','bernoulli','huberized','poisson','adaboost')

ResultsDist=list()
for(i in 1:length(DistSearch)){
  print(paste0('Dist ', i,' of ', length(DistSearch)))
  #Train model
  GBMModel=gbm(formula=Survived ~ ., data=Trainset, n.trees=100000, distribution=DistSearch[i])
  ...
  #Other metrics
  ResultsDist[[DistSearch[i]]]=c(confusionMatrix(factor(ifelse(test=Predicted[Chosen]>=0.5, yes=1, no=0)),
...
#Final model
#Train model
GBMModel=gbm(formula=Survived ~ ., data=Trainset, n.trees=100000, distribution='adaboost')
summary(GBMModel)
...
write_delim(x=SubV, path='R/Sub_V2.csv', delim=',')
...
if(RE){
  ...
  system('kaggle competitions submit -c titanic -f R/Sub_V2.csv -m "V2 Submission from API"')
}
